...
const mutations = {
  ...
  SET_ERROR_WEATHER(state, bStatus) {
    state.weatherError = bStatus;
  }
}
...
const actions = {
  setSliderInterval({
    ...
    state.slider.interval = setInterval(() => {
      state.slider.slideIndex += 1;
      dispatch("runCarosuel", state.slider.slideIndex)
    }, 4000)
  },
  runCarosuel({
    dispatch,
    ...
    if (n > slides.length) {
      commit("SET_SLIDE_INDEX", 1)
    }
    if (n < 1) {
      commit("SET_SLIDE_INDEX", slides.length)
    }
  ...
  getTodayFn({
    ...
    switch (dayOfTheWeek) {
      case 0:
        todayDate.dayDesc = i18n.t("news.sunday");
        break
      case 1:
        todayDate.dayDesc = i18n.t("news.monday");
        break
      case 2:
        todayDate.dayDesc = i18n.t("news.tuesday");
        break
      case 3:
        todayDate.dayDesc = i18n.t("news.wednesday");
        break
      case 4:
        todayDate.dayDesc = i18n.t("news.thursday");
        break
      case 5:
        todayDate.dayDesc = i18n.t("news.friday");
        break
      case 6:
        todayDate.dayDesc = i18n.t("news.saturday");
        break
    }
    if (hour > 6 && hour < 21) {
